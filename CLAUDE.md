...
Every published quote needs an accurate attribution. Two quotes that were originally shown on the Ask G-Pop page turned out to be from Howard's team (not Howard), so they were pulled rather than misattributed to him; that page carries a "Quote pending" note instead, same as Rap Frogs, whose client hasn't given a quote yet. Don't attribute a quote to someone who didn't say it, and don't resurrect a pulled quote without the right person's name and their organization's okay confirmed first.
...
- **Case studies** live at `src/case-studies/`: `index.njk` (three short cards linking out) plus one full page per client (`love-aid-foundation.njk`, `ask-g-pop.njk`, `rap-frogs.njk`), not a collection like articles. Each full page: a breadcrumb (`Case studies → [org]`), an eyebrow reading `[Person] · [Organization]`, the serif headline, a `.case-facts` strip (client type, status, started), a `.stat-row` of real numbers pulled out large, a two-column `.split` ("Sound familiar?" questions on the narrow side, a `.promise` checklist of what landed on the wide side, the width difference is the argument), the client's quotes with a `<cite>` attribution line under each, a "what this proves" block, a weekly delivery log with the most recent six weeks visible and everything earlier behind a native `<details>`/`<summary>` disclosure (no JavaScript), the booking CTA, and a closing list of links to the other case studies. The homepage's Case studies section links to each page by name.
  ...
  - When a client's quote doesn't exist yet, or was pulled for accuracy, say so plainly with the existing "Quote pending" treatment rather than leaving an empty gap or inventing one.
  ...
  - Follow the same rhythm when adding a case study: reuse `.prose`, `.log-*`, `.case-facts`, `.stat-row`, `.split`, `.promise`, `.breadcrumb`, and `.case-crosslinks` in `src/styles.css` rather than inventing new ones.
