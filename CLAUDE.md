...
8. Proof. Three anonymized client stories, one line each, linking to their own section on `/proof/`. See section 8.
...
**Proof.** Points to `/proof/` (`src/proof.njk`), one page holding three real, anonymized client stories stacked as cards (`#mission`, `#wisdom`, `#character`), built from Christopher's own weekly delivery logs. Names stay off both the homepage cards and `/proof/` itself until each client signs off in writing; that's the deliberate anonymization policy set in the source drafts, not a placeholder waiting on something. Once a client does sign off, that card gets their name, organization, and photo added: it doesn't need a new page built. Don't add a fourth story or embellish a real one with anything not in the delivery logs.
...
- **Proof** is one page, `src/proof.njk`, not a collection like articles: an intro with a plain (non-sticky) jump list to each client, then three stacked cards, each an eyebrow and headline, a `.case-facts` strip (client type, status, started), a `.stat-row` of real numbers pulled out large, a two-column `.split` ("Sound familiar?" questions on the narrow side, a `.promise` checklist of what landed on the wide side, the width difference is the argument), the client's quotes, a "what this proves" block, and a weekly delivery log with the most recent six weeks visible and everything earlier behind a native `<details>`/`<summary>` disclosure (no JavaScript). Log entries use `.log-num` for the week number, echoing the numbered rhythm from "how it works" rather than reading as a flat list. The homepage's Proof section links to each card's anchor by name.
  - `.stat-row` numbers are set in **rust, never orange**: orange text fails contrast on this page's cream and band grounds. Every number must already be a verified fact from that client's delivery log; never invent one to round out a row.
  - No sticky elements anywhere on this page (no sticky sub-nav, no sticky CTA bar). It fails the "would this look out of place printed and handed to someone?" test in section 3, even though it wouldn't touch the zero-JS rule. The repeated inline "Book my 15 minute call" button after each card is the mechanism instead.
  - No pulsing or otherwise animated status dot. `.log-dot`/`.log-status` stay static, per the no-animation-while-reading rule.
  - When a client's quote doesn't exist yet, say so plainly (see Client three's "Quote pending" line) rather than leaving an empty gap or inventing one.
  - Follow the same rhythm when adding a card or a fourth story: reuse `.prose`, `.log-*`, `.case-facts`, `.stat-row`, `.split`, and `.promise` in `src/styles.css` rather than inventing new ones.
