...
- **Case studies** live at `src/case-studies/`: `index.njk` (three short cards linking out) plus one full page per client (`love-aid-foundation.njk`, `ask-g-pop.njk`, `rap-frogs.njk`), not a collection like articles. Each full page: a breadcrumb (`Case studies → [org]`), an eyebrow reading `[Person] · [Organization]`, the serif headline, a `.case-facts` strip (client type, status, started), a `.stat-row` of real numbers pulled out large, a two-column `.split` ("Sound familiar?" questions on the narrow side, a `.promise` checklist of what landed on the wide side, the width difference is the argument), the client's quotes with a `<cite>` attribution line under each when a real, attributed quote exists, a "what this proves" block, a weekly delivery log with the most recent six weeks visible and everything earlier behind a native `<details>`/`<summary>` disclosure (no JavaScript), the booking CTA, and a closing list of links to the other case studies. The homepage's Case studies section links to each page by name.
  ...
  - When a client's quote doesn't exist yet, or was pulled for accuracy, leave the quote block out entirely rather than filling it with a placeholder note or inventing one.
