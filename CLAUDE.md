...
- **Case studies** live at `src/case-studies/`: `index.njk` (three short cards linking out, page heading "The work so far") plus one full page per client (`love-aid-foundation.njk`, `ask-g-pop.njk`, `rap-frogs.njk`), not a collection like articles. Each full page: a breadcrumb (`Home → Case studies → [org]`), an eyebrow reading `[Person] · [Organization]`, the serif headline, a `.case-facts` strip (client since, with a small "N months and running" sub-line, then client type), a two-column `.split.split-swapped` ("Here is what landed" on the wide left side, "Sound familiar?" questions on the narrow right side, with the first CTA nested inside that right column, directly under the questions, not spanning full width below both columns), the client's quotes with a `<cite>` attribution line under each when a real, attributed quote exists, a "what this proves" block, a second CTA, a weekly delivery log collapsed entirely behind one native `<details>`/`<summary>` disclosure closed by default (no JavaScript), a short first-person note from Christopher next to the reused homepage portrait, a third and closing CTA, and a closing list of links to the other case studies. The homepage's Case studies section (heading "The work so far") links to each page by name.
