...
- **Case studies** live at `src/case-studies/`: `index.njk` (three short cards linking out, page heading "The work so far") plus one full page per client (`love-aid-foundation.njk`, `ask-g-pop.njk`, `rap-frogs.njk`), not a collection like articles. Each full page: a breadcrumb (`Home → Case studies → [org]`), an eyebrow reading `[Person] · [Organization]`, the serif headline, a `.case-facts` strip (client since, with a small "N months and running" sub-line, then client type), and a two-column `.split.split-swapped`: "Here is what landed" on the wide left side, and on the narrow right side, "Sound familiar?" questions followed by the first CTA and, when a real quote exists, the client's quotes with a `<cite>` attribution line under each, all nested inside that narrow column rather than spanning full width below both columns. After that: a "what this proves" block, a weekly delivery log collapsed entirely behind one native `<details>`/`<summary>` disclosure closed by default (no JavaScript), a short first-person note from Christopher next to the reused homepage portrait, a second and closing CTA, and a closing list of links to the other case studies. The homepage's Case studies section (heading "The work so far") links to each page by name.
  ...
  - **Two CTAs per page, not three.** A third CTA used to sit between "What this proves" and the weekly log; Christopher removed it on direct feedback as unnecessary. What's left: one nested inside "Sound familiar?" (see above) and one closing CTA above the case-crosslinks. Don't re-add a mid-page CTA between "what this proves" and the weekly log.
  - **Every CTA on a case study page carries the same three lines**: the "Book my 15 minute call" button, `or call 310-703-6003` underneath it (third person, per the phone-number convention above), and the risk-reversal line `Month to month. Everything built is yours.` (a shortened version Christopher preferred over the original "Month to month. One email ends it. Everything built stays yours.", after seeing both live; the price section's own paragraph on the homepage is untouched). Reuse `.content-cta`, `.call-alt`, and `.risk-reversal`.
  - The closing block reuses `/christopher.jpg` at a smaller size (`.portrait.portrait-sm`) next to one first-person sentence from Christopher, inside `.author-note`, directly above the page's final CTA. Don't add a different photo or a synthetic person: the portrait already on the homepage is the only trust photo this site uses, per section 3.
