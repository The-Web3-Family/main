...
1. Header. Logo only. No navigation, no social links. Nothing leaves the homepage except the phone number and the logo, which links to `/` (a no-op there, a way home from every other page). On subpages (case studies, articles) the header logo and the breadcrumb's first crumb both link to `/`, added on direct request; this is the one deliberate exception to the "nothing to click but the phone number and the one call to action" rule, since it's a single way home rather than a nav bar to multiple destinations. Don't turn it into an actual nav bar without a separate explicit conversation.
...
- Ships zero client-side JavaScript beyond the booking embed and Microsoft Clarity (analytics). No animation libraries, sliders, or other analytics tools. Eleventy's job is finished at build time; nothing it does should add a runtime script beyond those two. The booking embed now has two modes on the site: the homepage's inline widget (unchanged), and, on the three case study pages, Calendly's official popup widget (their `widget.css`/`widget.js`, plus an `onclick="Calendly.initPopupWidget(...)"` on the CTA link, with a plain link to the Calendly page as the href fallback if JavaScript is off). Both modes are the same one third-party booking product, not a second dependency; the popup assets are loaded only on pages that set `calendlyPopup: true` in front matter (see `base.njk`), not sitewide.
...
- **Case studies** live at `src/case-studies/`: `index.njk` (three short cards linking out, page heading "The work so far") plus one full page per client (`love-aid-foundation.njk`, `ask-g-pop.njk`, `rap-frogs.njk`), not a collection like articles. Each full page: a breadcrumb (`Home → Case studies → [org]`), an eyebrow reading `[Person] · [Organization]`, the serif headline, a `.case-facts` strip (client since, with a small "N months and running" sub-line, then client type), and a two-column `.split.split-swapped`: "Here is what landed" on the wide left side, and on the narrow right side, "Sound familiar?" questions followed by the page's one CTA, nested inside that narrow column rather than spanning full width below both columns. After the two-column split (full width, spanning both columns): when a real quote exists, the client's quotes with a `<cite>` attribution line under each (moved back out of the narrow column after a brief experiment nesting them there), a "what this proves" block, a weekly delivery log collapsed entirely behind one native `<details>`/`<summary>` disclosure closed by default (no JavaScript), a short first-person note from Christopher next to the reused homepage portrait, and a closing list of links to the other case studies. The homepage's Case studies section (heading "The work so far") links to each page by name.
  ...
  - **Tenure is framed as "Client since [Month Year]"** everywhere (fact strip, index cards, homepage teaser cards), never as "Running, week N." The week count lives only inside the weekly log's status line, e.g. "Week 25, and next Friday is already in motion." The fact strip's "Client since" column also carries a small `.case-fact-sub` line ("5 months and running") directly under the date: real, hand-computed elapsed time from the client's start month to the most recent weekly log entry, not a live JavaScript counter. A live counter was asked for once; it stays off the table because this site ships zero client-side JavaScript beyond the booking embed and Clarity (section 9) and a hand-updated date is what every other date on this site already is.
  ...
  - **One CTA per page, not three.** These pages went from three CTAs down to one, across two rounds of direct feedback: first the CTA between "What this proves" and the weekly log was cut, then the closing CTA near the bottom was cut too. What's left is the CTA nested inside "Sound familiar?" (see above), the only one on the page. Don't add a CTA elsewhere on these pages without a fresh, explicit request; the trend across this feedback was fewer, not more.
  - **The CTA carries three lines**: the "Book my 15 minute call" button, `or call 310-703-6003` underneath it (third person, per the phone-number convention above), and the risk-reversal line `Month to month. Everything built is yours.` (a shortened version Christopher preferred over the original "Month to month. One email ends it. Everything built stays yours.", after seeing both live; the price section's own paragraph on the homepage is untouched). Reuse `.content-cta`, `.call-alt`, and `.risk-reversal`.
  - **The CTA button opens Calendly's popup widget** instead of linking to the homepage's `/#booking` section, per direct request. Front matter sets `calendlyPopup: true` (see `base.njk`, and section 9) to load Calendly's popup assets on that page only; the button itself is `<a href="https://calendly.com/aweb3dad/15-minute-greeting" onclick="Calendly.initPopupWidget({url:'...'});return false;">`, so it still goes straight to the real booking page if JavaScript is off. The case-studies index page's own CTA is unchanged (still links to `/#booking`); only the three detail pages got the popup.
  ...
  - The closing block reuses `/christopher.jpg` at a smaller size (`.portrait.portrait-sm`) next to one first-person sentence from Christopher, inside `.author-note`, at the very end of the page, right before the cross-links to the other case studies. It no longer sits next to a CTA of its own, since the page's one CTA now lives higher up, inside "Sound familiar?". Don't add a different photo or a synthetic person: the portrait already on the homepage is the only trust photo this site uses, per section 3.
  - No sticky elements anywhere on these pages (no sticky sub-nav, no sticky CTA bar). It fails the "would this look out of place printed and handed to someone?" test in section 3, even though it wouldn't touch the zero-JS rule. The repeated inline "Book my 15 minute call" button is the mechanism instead.
  - No pulsing or otherwise animated status dot. `.log-dot`/`.log-status` stay static, per the no-animation-while-reading rule. The weekly log's disclosure arrow may rotate on open (a hover/interaction state, not a passive-reading animation) and the card grid may lift slightly on hover; both are plain CSS transitions, already covered by the sitewide `prefers-reduced-motion` rule rather than gated individually.
