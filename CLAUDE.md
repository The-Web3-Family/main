...
- **Case studies** live at `src/case-studies/`: `index.njk` (three short cards linking out, page heading "The work so far") plus one full page per client (`love-aid-foundation.njk`, `ask-g-pop.njk`, `rap-frogs.njk`), not a collection like articles. Each full page: a breadcrumb (`Case studies → [org]`), an eyebrow reading `[Person] · [Organization]`, the serif headline, a `.case-facts` strip (client type, client since), a `.stat-row` of real numbers pulled out large, a two-column `.split` ("Sound familiar?" questions on the narrow side, a `.promise` checklist of what landed on the wide side, the width difference is the argument), a CTA, the client's quotes with a `<cite>` attribution line under each when a real, attributed quote exists, a "what this proves" block, a second CTA, a weekly delivery log collapsed entirely behind one native `<details>`/`<summary>` disclosure closed by default (no JavaScript), a short first-person note from Christopher next to the reused homepage portrait, a third and closing CTA, and a closing list of links to the other case studies. The homepage's Case studies section (heading "The work so far") links to each page by name.
  - **Tenure is framed as "Client since [Month Year]"** everywhere outside the weekly log itself (fact strip, index cards, homepage teaser cards), never as "Running, week N." The week count lives only inside the weekly log's status line, e.g. "Week 25, and next Friday is already in motion." A "N weeks running" entry in `.stat-row` is redundant with that and with "Client since" and has been removed from all three pages; don't add one back. The other `.stat-row` numbers (things delivered, dollars saved, and similar) stay.
  - **Every CTA on a case study page carries the same three lines**: the "Book my 15 minute call" button, `or call 310-703-6003` underneath it (third person, per the phone-number convention above), and the risk-reversal line `Month to month. One email ends it. Everything built stays yours.` (this restates the homepage price section's own claim, not a new one). Reuse `.content-cta`, `.call-alt`, and `.risk-reversal`; add `.content-cta-lead` on a CTA that opens a section, to zero out its top margin.
  ...
  - `.stat-row` numbers are set in **rust, never orange**: orange text fails contrast on these pages' cream and band grounds. Every number must already be a verified fact from that client's delivery log; never invent one to round out a row.
  ...
  - Follow the same rhythm when adding a case study: reuse `.prose`, `.log-*`, `.case-facts`, `.stat-row`, `.split`, `.promise`, `.breadcrumb`, `.author-note`, and `.case-crosslinks` in `src/styles.css` rather than inventing new ones.
